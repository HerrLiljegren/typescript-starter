...
describe('POST api/v1/heroes/:id', () => {
	it('updates Spider-Mans gender to be female', () => {
		return chai.request(app).post('/api/v1/heroes/2')
			.send({ gender: "female" })
			.then(res => {
				expect(res.body.hero.gender).to.equal('female');
			});
	});
});